...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        data_loss = np.sum(- scores[range(N), y] + np.log(rows)) / N
        # Regularization
        ...
        ddataloss = (np.exp(scores).T / rows).T
        # Gradient of -scores[range(N), y]:
        ys = np.zeros(ddataloss.shape)
        ys[range(N), y] = 1
        ddataloss -= ys
        # / N term
        ddataloss /= N
        ...
        # Chain rule
        dscores = ddataloss.dot(W2.T)

        # Relu gradient
        dlayer2 = dscores * (layer1>=0)

        dW1 = X.T.dot(dlayer2) + reg * W1
        dW2 = layer2.T.dot(ddataloss) + reg * W2

        # Same as matrix multiplication with 1-vector, chain rule works out
        db1 = np.sum(dlayer2, axis=0)
        db2 = np.sum(ddataloss, axis=0)
